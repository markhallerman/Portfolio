...
struct EditProjectView: View {
    ...
    var body: some View {
        if project.isDeleted || project.isFault {
            ...
            Form {
                ...
                Section(header: Text("Project Reminders")) {
                    Toggle("Show reminders", isOn: $remindMe.animation().onChange(update))
                        .alert(isPresented: $showingNotificationsError) {
                            Alert(title: Text("Notifications Error"), message: Text("There was a problem. Please check that you have notifications enabled."), primaryButton: .default(Text("Check Settings"), action: showAppSystemSettings), secondaryButton: .cancel())
                        }
                ...
                Section(footer: Text("Closing a project moves it from the Open to the Closed tab; Deleting it removes it entierly.")) {
    ...
    func toggleClosed() {
        ...
        if project.closed {
            playHapticShortThenLongWindingDown()
        }
    }
    ...
    func playHapticShortThenLongWindingDown() {
        do {
            try engine?.start()
            ...
            // playing haptics didnt work, but its ok.
        }
    }
